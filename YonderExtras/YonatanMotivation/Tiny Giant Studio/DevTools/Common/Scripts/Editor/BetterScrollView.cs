...
namespace TinyGiantStudio.DevTools
{
...
#else
    public class BetterScrollView : ScrollView
#endif
...
#else
        public new class UxmlFactory : UxmlFactory<BetterScrollView, UxmlTraits>
        { }

        public new class UxmlTraits : ScrollView.UxmlTraits
        {
            private UxmlBoolAttributeDescription Interactable = new UxmlBoolAttributeDescription { name = "Interactable", defaultValue = true };
            private UxmlBoolAttributeDescription IgnoreChildren = new UxmlBoolAttributeDescription { name = "IgnoreChildren", defaultValue = false };

            public UxmlTraits() : base()
            {
                Interactable.defaultValue = true;
            }
            ...
            {
                base.Init(ve, bag, cc);
                ((BetterScrollView)ve).isInteractable = Interactable.GetValueFromBag(bag, cc);
                ((BetterScrollView)ve).IgnoreChildren = IgnoreChildren.GetValueFromBag(bag, cc);
            }
        }
        public bool isInteractable = true;
    ...
    int _id;
        ...
        public BetterScrollView() : this(ScrollViewMode.Vertical)
        {
        }

        public BetterScrollView(ScrollViewMode scrollViewMode) : base(scrollViewMode)
        {
            DoRegisterCallbacks();
        }

        private VisualElement MouseOwner => this;
        ...
        private void HandleDrag(IMouseEvent e)
        {
...
#endif
            if (!worldBound.Contains(e.mousePosition) || e.button == 1)
            {
#if DRAG_LOGGING
        ...
        // Copied from Unity source code: https://github.com/Unity-Technologies/UnityCsReference/blob/master/ModuleOverrides/com.unity.ui/Core/Controls/ScrollView.cs
