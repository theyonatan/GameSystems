...
public class StoryCharacter : MonoBehaviour
{
    ...
    public void GoTo(string storyObjectId, float speed = 4f, bool withRotation=false, bool withAnimation=true, float gotoDistance=0.4f, bool continueStoryWhileMoving=false)
    {
        ...
        _storyExecuter.addAction(new GoTo(transform, gotoObject.GetLocation(), speed, _navMeshAgent, animator, gotoDistance, continueStoryWhileMoving: continueStoryWhileMoving));
        if (withRotation)
